...
public class BSoftBodyPartOnSkinnedMesh : BSoftBody
{
	...
	private BoneAndNode[] bone2idxMap;
	...
	public void BindBonesToSoftBodyAndNodesToAnchors()
	{
		...
		for (int i = 0; i < cols.Length; i++)
		{
			for (int j = 0; j < anchors.Length; j++)
			{
				if (cols[i].g > anchors[j].colRangeFrom && cols[i].g < anchors[j].colRangeTo)
				{
	...
	private void _addEdges(int p, int a, int b, List<Edge> edges, Vector3[] ns, Vector3[] vs)
	{
    ...
    */
	...
	private Quaternion WahbasSolution(Vector3 b1, Vector3 b2, Vector3 r1, Vector3 r2)
	{
